        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {plans.map((plan, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className={`relative rounded-3xl border p-8 shadow-lg hover:shadow-2xl transition-all duration-500 bg-white 
                ${plan.popular ? "border-[#2A4E3B] scale-105" : "border-gray-200"}
              `}
            >
              {/* Popular Badge */}
              {plan.popular && (
                <span className="absolute -top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-green-500 to-green-700 text-white px-5 py-1 rounded-full text-sm font-semibold shadow-lg animate-pulse">
                  Most Popular
                </span>
              )}

              <h3 className="text-2xl md:text-3xl font-bold text-[#2A4E3B] mb-2">
                {plan.title}
              </h3>
              ...
              <div className="text-4xl md:text-5xl font-extrabold text-gray-900 mb-6 flex items-baseline">
                {plan.price}
              ...
              <a
                ...
                className={`block text-center px-6 py-3 rounded-lg font-semibold transition-all duration-300
                  ${
                    plan.popular
                      ? "bg-gradient-to-r from-green-500 to-green-700 text-white hover:from-green-600 hover:to-green-800 shadow-lg"
